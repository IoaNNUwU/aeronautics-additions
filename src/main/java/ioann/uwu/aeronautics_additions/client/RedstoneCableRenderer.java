package ioann.uwu.aeronautics_additions.client;
...
public class RedstoneCableRenderer {
    ...
    public static void render(final SmartBlockEntity be, final RopeStrandHolderBehavior ropeHolder, final float partialTick, final PoseStack ps, final MultiBufferSource buffer) {
        ...
        final SuperByteBuffer middle = CachedBuffers.partialFacing(AAPartialModels.REDSTONE_CABLE, AllBlocks.ROPE.getDefaultState(), Direction.NORTH);
        final SuperByteBuffer knot = CachedBuffers.partialFacing(AAPartialModels.REDSTONE_CABLE_KNOT, AllBlocks.ROPE.getDefaultState(), Direction.NORTH);
        ...
        if (ropeHolder.ownsRope() && rope != null && ropeHolder.getClientStrand() != null) {
            ...
            final ObjectArrayList<RopeStrandRenderer.RopeRenderPoint> ropeRenderPoints = buildRenderPoints(partialTick, points);

            if (ropeRenderPoints.isEmpty()) return;

            ps.pushPose();
            for (int i = 1; i < ropeRenderPoints.size(); i++) {
                final RopeStrandRenderer.RopeRenderPoint renderPoint0 = ropeRenderPoints.get(i - 1);
                final RopeStrandRenderer.RopeRenderPoint renderPoint1 = ropeRenderPoints.get(i);
            ...
            final RopeStrandRenderer.RopeRenderPoint last = ropeRenderPoints.getLast();
            if (containingPose != null) {
    ...
    private static void renderOutline(final PoseStack ps, final MultiBufferSource buffer, final float rad, final ObjectArrayList<RopeStrandRenderer.RopeRenderPoint> ropeRenderPoints, final BlockPos ownerPos) {
        ...
        for (int i = 0; i < ropeRenderPoints.size() + 1; i++) {
            final RopeStrandRenderer.RopeRenderPoint renderPoint0 = ropeRenderPoints.get(Math.max(0, i - 1));
            final RopeStrandRenderer.RopeRenderPoint renderPoint1 = ropeRenderPoints.get(Math.min(ropeRenderPoints.size() - 1, i));
    ...
    private static @NotNull ObjectArrayList<RopeStrandRenderer.RopeRenderPoint> buildRenderPoints(final float partialTick, final List<ClientRopePoint> inputPoints) {
        ...
        ropeRenderPoints.add(new RopeStrandRenderer.RopeRenderPoint(new Quaternionf(runningRotation), points.getLast().renderPos(partialTick, new Vector3d())));
